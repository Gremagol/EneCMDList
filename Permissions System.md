...
------------------------------
Permissions are very handy at setting who can use what commands in a server. By default, the NSFW module is blocked, but nothing else is. If something is a bot owner only command, it can only be ran by Gremagol. 
...
With the permissions system it possible to restrict who can skip the current song, pick Coins or use the NSFW module.
